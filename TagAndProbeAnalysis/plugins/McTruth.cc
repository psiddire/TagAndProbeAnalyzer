...
class McTruth : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
...
void
McTruth::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  for(auto&& muon : *(muonColl.product())) {
    ...
    //if(std::abs(gmu->pdgId()) != 13 || gmu->status() != 1) continue;
    ...
    h_rec_pt->Fill(gmu->pt());
    h_rec_eta->Fill(gmu->eta());
    ...
    //  - Loose 
    if(muon.isLooseMuon()) {
      h_rec_loose_pt->Fill(gmu->pt());
      h_rec_loose_eta->Fill(gmu->eta());
      h_rec_loose_nvtx->Fill(nGoodVtx);
    }

    //  - Medium 
    if(muon.isMediumMuon()) {
      h_rec_medium_pt->Fill(gmu->pt());
      h_rec_medium_eta->Fill(gmu->eta());
      h_rec_medium_nvtx->Fill(nGoodVtx);
    }

    //  - Tight
    if(muon.isTightMuon(*goodVtx)==false) continue; // if it's not tight, nothing else to do 
    h_rec_tight_pt->Fill(gmu->pt());
    h_rec_tight_eta->Fill(gmu->eta());
    ...
    double corriso = (pfR04.sumChargedHadronPt+std::max(0.0,pfR04.sumNeutralHadronEt+pfR04.sumPhotonEt-0.5*pfR04.sumPUPt))/muon.pt();

    if(corriso>0.12) continue; // not isolated, nothing else to do
    h_rec_tight_iso_pt->Fill(gmu->pt());
    h_rec_tight_iso_eta->Fill(gmu->eta());
    ...
    h_rec_tight_iso_hlt_pt->Fill(gmu->pt());
    h_rec_tight_iso_hlt_eta->Fill(gmu->eta());
...
bool McTruth::matchTriggerObject(const pat::Muon& mu,
					      const std::map<TString, size_t>& trigList,
					      const edm::TriggerNames& trigNames, 
					      edm::Handle<pat::TriggerObjectStandAloneCollection> trigObjs,
					      edm::Handle<edm::TriggerResults> trigBits) {
