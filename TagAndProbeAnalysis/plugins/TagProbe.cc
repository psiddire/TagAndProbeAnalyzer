...
class TagProbe : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit TagProbe(const edm::ParameterSet&);
  ~TagProbe();
...
//
TagProbe::TagProbe(const edm::ParameterSet& iConfig)

{
  ...
  edm::InputTag trigObjTag("slimmedPatTrigger");
...
void
TagProbe::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  //for(auto&& test : *(muonColl.product())) {
  //  std::cout<<__LINE__<< " Muon Pt " << test.pt() << " eta " << test.eta() << " q " << test.charge() << " Tight " << test.isTightMuon(*goodVtx) << " Stand " << test.isStandAloneMuon() << " Global " << test.isGlobalMuon() <<std::endl;
  //}
  ...
  for(auto&& tag : *(muonColl.product())) {
    ...
    // 
    //  - pt, eta 
    if( tag.pt() < 25 || fabs(tag.eta()) > 2.4  ) continue;
    // 
    //  - ID 
    if( !tag.isTightMuon(*goodVtx) ) continue;
    
    // 
    //  - impact parameters (tighter than in ID)
    bool ip = fabs(tag.muonBestTrack()->dxy(goodVtx->position()))<0.05 && fabs(tag.muonBestTrack()->dz(goodVtx->position()))<0.1; 
    if( !ip ) continue;
    // 
    //  - isolation 
    double tagcorriso = 99999.;
    if(tag.isIsolationValid()) {
      reco::MuonPFIsolation pfR04 = tag.pfIsolationR04();
      tagcorriso = (pfR04.sumChargedHadronPt+std::max(0.0,pfR04.sumNeutralHadronEt+pfR04.sumPhotonEt-0.5*pfR04.sumPUPt))/tag.pt();
    }
    if( tagcorriso > 0.12 ) continue; 
    //
    // trigger matching 
    bool tagTrigger = matchTriggerObject(tag, tagTriggerIdxList, triggerNames, triggerObjects, triggerBits);
    if( !tagTrigger ) continue; 

    // Good tag found!
    // Save tag charge and p4, for later 
    int tagq = tag.charge();
    //auto tagp4 = tag.p4(); 
    ...
    for(auto&& probe : *(muonColl.product())) {
      // Probe selection:
      //
      //std::cout<<__LINE__<< " Probe Pt" << probe.pt() << " " << probe.eta() << " " << probe.charge() << " " << probe.isStandAloneMuon() <<std::endl;
      //  - opposite electric charge sign w.r.t. tag
      if( tagq==probe.charge() ) continue; 
      // 
      //  - skip standalone-only muons 
      if( probe.isStandAloneMuon() && !probe.isGlobalMuon() ) continue; 
      // 
      //  - pt > 20,  |eta| < 2.4 
      ...
      TLorentzVector pfz;
      pfz.SetPxPyPzE(tag.px()+probe.px(),tag.py()+probe.py(),tag.pz()+probe.pz(),tag.energy()+probe.energy());
      double tagprobeMass = pfz.M(); 
	...
	if(probe.isIsolationValid()) {
	  const reco::MuonPFIsolation pfR04 = probe.pfIsolationR04();
	  double corriso = (pfR04.sumChargedHadronPt+std::max(0.0,pfR04.sumNeutralHadronEt+pfR04.sumPhotonEt-0.5*pfR04.sumPUPt))/probe.pt();
	  if( corriso < 0.12 ) h_tag_probe_iso_mass->Fill(tagprobeMass); // Fill ... 
	}
	bool probeTrigger = matchTriggerObject(probe, triggerIdxList, triggerNames, triggerObjects, triggerBits);
	if( probeTrigger==true ){
      ...
      if( tagprobeMass > 96 || tagprobeMass < 86 ) continue; 
	...
	h_rec_medium_pt->Fill(probe.pt());
      ...
      if( corriso > 0.12 ) continue; // not isolated, nothing else to do, you can skip to next event 
      // Fill plots for probes that are isolated 
...
bool TagProbe::matchTriggerObject(const pat::Muon& mu,
					      const std::map<TString, size_t>& trigList,
					      const edm::TriggerNames& trigNames, 
					      edm::Handle<pat::TriggerObjectStandAloneCollection> trigObjs,
					      edm::Handle<edm::TriggerResults> trigBits) {
